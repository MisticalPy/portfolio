...
(() => {
  ...
  const stylesheet =
    ...
    // touch-action:auto (not none) so a finger drag over a filled slot scrolls
    // the page on mobile — the slot is read-only outside the editor, and a
    // blanket touch-action:none made every image a dead zone for scrolling.
    // Pan/pinch capture is re-enabled only while reframing (rule below).
